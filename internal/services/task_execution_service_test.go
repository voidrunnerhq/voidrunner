...
func TestNewTaskExecutionService(t *testing.T) {
	...
	// Create a test config with Docker settings
	cfg := &config.Config{
		Docker: config.DockerConfig{
			PythonExecutorImage: "python:3.11-alpine",
			BashExecutorImage:   "alpine:latest",
			DefaultMemoryMB:     128,
	...
	service := NewTaskExecutionService(conn, dockerClient, logger, cfg)

	assert.NotNil(t, service)
	assert.Equal(t, conn, service.conn)
	assert.Equal(t, dockerClient, service.dockerClient)
	assert.Equal(t, logger, service.logger)
	assert.Equal(t, cfg, service.appConfig)
}
